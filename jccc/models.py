...
class Allocation(models.Model):
	...
	def __str__(self):
		return '%d:[$%.02f] %s -> %s' % (self.year, self.value, str(self.source), str(self.recipient))
...
def check_stuff(sender, instance, **kwargs):
	...
	try:
		edit_list = instance.editors
	except ValueError:
		edit_list = []

	if not instance.contact.username in edit_list:
		edit_list.append(instance.contact.username)

	instance.editors = json.dumps(edit_list)
